...
contract Roles is IRoles, AccessControl {
    ...
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, _msgSender()), "Roles: caller is not an admin");
        _;
    }

    modifier onlyCS() {
        require(hasRole(CS_ROLE, _msgSender()), "Roles: caller is not a CS for any club");
        _;
    }

    modifier onlyAdminOrModerator() {
        require(hasRole(ADMIN_ROLE, _msgSender()) || hasRole(MODERATOR_ROLE, _msgSender()), "Roles: caller is not admin or moderator");
        _;
    }

    modifier onlyModerator() {
        require(hasRole(MODERATOR_ROLE, _msgSender()), "Roles: caller is not a moderator for this club");
        _;
    }

    modifier onlyFan() {
        require(hasRole(FAN_ROLE, _msgSender()), "Roles: caller is not a fan of any club");
        _;
    }
    ...
    function grantCSRole(address account) public onlyAdminOrModerator() {
        require(!hasRole(CS_ROLE, account), "User is not CS role for this club");
    ...
    function revokeCSRole(address account) public onlyAdminOrModerator {
        require(hasRole(CS_ROLE, account), "User does not have a CS role for this club");
    ...
    function isStaff(address account) external view returns (bool) {
        return hasRole(ADMIN_ROLE, account) || hasRole(MODERATOR_ROLE, account) || hasRole(CS_ROLE, account);
    }
